...
class ElectricSlide
  class CallQueue
    ...
    def checkout_agent
      ...
      set_agent_presence :busy
      agent
    end

    def set_agent_presence(status)
      if agent.presence != status
        agent.presence = status
        agent.callback :status_changed, self, agent.call, status
      end
    end
